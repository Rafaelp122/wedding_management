from django.db.models import QuerySet
from ninja.pagination import paginate
from ninja_extra import (
    ControllerBase,
    api_controller,
    http_delete,
    http_get,
    http_patch,
    http_post,
)
from pydantic import UUID4

from apps.core.constants import MUTATION_ERROR_RESPONSES, READ_ERROR_RESPONSES
from apps.scheduler.models import Event, Task
from apps.scheduler.schemas import (
    EventIn,
    EventOut,
    EventPatchIn,
    TaskIn,
    TaskOut,
    TaskPatchIn,
)
from apps.scheduler.services import EventService, TaskService


@api_controller("/scheduler", tags=["Scheduler"])
class SchedulerController(ControllerBase):
    """
    Controller para gestão de cronograma e checklist.
    Centraliza a manipulação de eventos e tarefas em um único contexto.
    """

    def __init__(self, event_service: EventService, task_service: TaskService):
        self.event_service = event_service
        self.task_service = task_service

    # --- Endpoints de Eventos (Agenda) ---

    @http_get("/events/", response=list[EventOut], operation_id="scheduler_events_list")
    @paginate
    def list_events(self, wedding_id: UUID4 | None = None) -> QuerySet[Event]:
        """
        Lista todos os eventos do cronograma do Planner logado.
        Pode ser filtrado por um casamento específico.
        """
        return self.event_service.list(self.context.request.user, wedding_id=wedding_id)

    @http_get(
        "/events/{uuid}/",
        response={200: EventOut, **READ_ERROR_RESPONSES},
        operation_id="scheduler_events_read",
    )
    def retrieve_event(self, uuid: UUID4) -> Event:
        """
        Retorna detalhes de um evento específico.
        """
        return self.event_service.get(self.context.request.user, uuid=uuid)

    @http_post(
        "/events/",
        response={201: EventOut, **MUTATION_ERROR_RESPONSES},
        operation_id="scheduler_events_create",
    )
    def create_event(self, payload: EventIn) -> tuple[int, Event]:
        """
        Cria um novo evento no cronograma.
        """
        event = self.event_service.create(
            self.context.request.user, payload.model_dump()
        )
        return 201, event

    @http_patch(
        "/events/{uuid}/",
        response={200: EventOut, **MUTATION_ERROR_RESPONSES},
        operation_id="scheduler_events_partial_update",
    )
    def partial_update_event(self, uuid: UUID4, payload: EventPatchIn) -> Event:
        """
        Atualiza parcialmente um evento.
        """
        return self.event_service.partial_update(
            self.context.request.user, uuid, payload.model_dump(exclude_unset=True)
        )

    @http_delete(
        "/events/{uuid}/",
        response={204: None, **MUTATION_ERROR_RESPONSES},
        operation_id="scheduler_events_delete",
    )
    def delete_event(self, uuid: UUID4) -> tuple[int, None]:
        """
        Remove um evento do cronograma.
        """
        self.event_service.delete(self.context.request.user, uuid)
        return 204, None

    # --- Endpoints de Tarefas (Checklist) ---

    @http_get("/tasks/", response=list[TaskOut], operation_id="scheduler_tasks_list")
    @paginate
    def list_tasks(self, wedding_id: UUID4 | None = None) -> QuerySet[Task]:
        """
        Lista tarefas e checklist.
        """
        return self.task_service.list(self.context.request.user, wedding_id=wedding_id)

    @http_post(
        "/tasks/",
        response={201: TaskOut, **MUTATION_ERROR_RESPONSES},
        operation_id="scheduler_tasks_create",
    )
    def create_task(self, payload: TaskIn) -> tuple[int, Task]:
        """
        Cria uma nova tarefa no checklist.
        """
        task = self.task_service.create(self.context.request.user, payload.model_dump())
        return 201, task

    @http_patch(
        "/tasks/{uuid}/",
        response={200: TaskOut, **MUTATION_ERROR_RESPONSES},
        operation_id="scheduler_tasks_partial_update",
    )
    def partial_update_task(self, uuid: UUID4, payload: TaskPatchIn) -> Task:
        """
        Atualiza uma tarefa (incluindo marcação de conclusão).
        """
        return self.task_service.partial_update(
            self.context.request.user, uuid, payload.model_dump(exclude_unset=True)
        )

    @http_delete(
        "/tasks/{uuid}/",
        response={204: None, **MUTATION_ERROR_RESPONSES},
        operation_id="scheduler_tasks_delete",
    )
    def delete_task(self, uuid: UUID4) -> tuple[int, None]:
        """
        Remove uma tarefa permanentemente.
        """
        self.task_service.delete(self.context.request.user, uuid)
        return 204, None
