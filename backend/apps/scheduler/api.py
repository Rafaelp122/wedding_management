...
@router.get("/{uuid}/", response=EventOut, operation_id="scheduler_events_read")
def get_event(request, uuid: UUID4):
    """
...
@router.post("/", response={201: EventOut}, operation_id="scheduler_events_create")
...
@router.patch(
    "/{uuid}/", response=EventOut, operation_id="scheduler_events_partial_update"
)
def partial_update_event(request, uuid: UUID4, payload: EventPatchIn):
    """
...
@router.delete("/{uuid}/", response={204: None}, operation_id="scheduler_events_delete")
def delete_event(request, uuid: UUID4):
    """
