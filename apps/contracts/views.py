...
from apps.contracts.mixins import ContractManagementMixin, ContractOwnershipMixin
...
class ContractsPartialView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        contracts_qs = Contract.objects.for_wedding(wedding).with_related()
...
class GenerateSignatureLinkView(ContractOwnershipMixin, ContractManagementMixin, View):
    """
    ...
    def get(self, request, contract_id):
        ...
        try:
            ...
            traceback.print_exc()
            return self.json_error(f"ERRO NO SERVIDOR: {e!s}")
    ...
    def post(self, request, contract_id):
        ...
        try:
            ...
            success, message = self.send_signature_email(contract, email_cliente)
            ...
            return self.json_error(f"Erro ao enviar e-mail: {e!s}")
...
class SignContractExternalView(ContractManagementMixin, TemplateView):
    ...
    def post(self, request, token):
        ...
        success, message = self.process_contract_signature(
            contract, signature_b64, request
        )

        if success:
            return render(
                request, "contracts/signature_success.html", {"contract": contract}
            )
...
def download_contract_pdf(request, contract_id):
    ...
    context = {"contract": contract, "qr_code": qr_base64}
...
class CancelContractView(ContractOwnershipMixin, ContractManagementMixin, View):
    """
...
class EditContractView(ContractOwnershipMixin, ContractManagementMixin, View):
    """
    ...
    def post(self, request, contract_id):
        ...
        success, message = self.update_contract_description(contract, new_description)
...
class UploadContractView(ContractOwnershipMixin, ContractManagementMixin, View):
    """
    ...
    def post(self, request, contract_id):
        ...
        success, message = self.upload_external_contract(contract, pdf_file)
