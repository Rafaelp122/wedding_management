...
class ContractAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações Básicas", {
            "fields": ("item", "description", "status", "token")
        }),
        ...
        ("Integridade e Arquivos", {
            "fields": ("integrity_hash", "final_pdf")
        }),
        ("Metadados", {
    ...
    def signature_progress(self, obj):
        ...
        if obj.planner_signature:
            signatures.append(
                '<span style="color: green;">✓ Cerimonialista</span>'
            )
        else:
            signatures.append(
                '<span style="color: gray;">○ Cerimonialista</span>'
            )

        if obj.supplier_signature:
            signatures.append(
                '<span style="color: green;">✓ Fornecedor</span>'
            )
        else:
            signatures.append(
                '<span style="color: gray;">○ Fornecedor</span>'
            )

        if obj.couple_signature:
            signatures.append(
                '<span style="color: green;">✓ Noivos</span>'
            )
        else:
            signatures.append(
                '<span style="color: gray;">○ Noivos</span>'
            )
