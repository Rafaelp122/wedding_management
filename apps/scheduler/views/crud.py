...
class EventListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        print(
            f"DEBUG EventListView: Buscando eventos para planner ID: {self.request.user.id}"
        )
...
class EventCreateView(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        print(
            f"DEBUG EventCreateView: Definindo planner {self.request.user.id} para novo evento."
        )
...
class EventDeleteView(EventPlannerOwnerMixin, SuccessMessageMixin, DeleteView):
    ...
    def get_object(self, queryset=None):
        ...
        print(
            f"DEBUG EventDeleteView: Preparando exclusão ID {obj.id} ('{self.object_title}')"
        )
        return obj
