...
class EventPlannerOwnerMixin(LoginRequiredMixin):
    ...
    def get_queryset(self):
        print(
            f"DEBUG EventPlannerOwnerMixin: Filtrando queryset para planner ID: {self.request.user.id}"
        )
        qs = super().get_queryset().filter(planner=self.request.user)
        print(
            f"DEBUG EventPlannerOwnerMixin: Queryset filtrada encontrada: {qs.count()} eventos."
        )
        return qs
