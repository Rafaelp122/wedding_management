...
export function DashboardOperations({ weddings }: DashboardOperationsProps) {
  ...
  // 2. Upcoming Weddings
  const displayWeddings = weddings.slice(0, 5);

  // 3. Urgent Tasks (Incomplete + (due_date <= today or null))
  const todayStr = new Date().toISOString().slice(0, 10);
  const urgentTasks = (tasksRes?.data?.items ?? [])
  ...
  const pendingContracts = (contractsRes?.data?.items ?? [])
