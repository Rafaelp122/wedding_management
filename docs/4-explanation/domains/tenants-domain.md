...
> **Hub de Domínio:** [tenants-domain](tenants-domain.md) | [multi-tenancy-strategy](../architecture/multi-tenancy-strategy.md)
...
- **Modelos de Dados:**
  - [tenant-model](../../3-reference/models/tenants/tenant-model.md): Modelo `Company` (`companies`) e a classe base abstrata `TenantModel`.
- **Managers:** `TenantManager` e `TenantQuerySet` com o método `.for_tenant(company)`. Veja [multi-tenancy-strategy](../architecture/multi-tenancy-strategy.md).
...
- **Store de Sessão:** `useAuthStore` contendo a `Company` ativa do usuário autenticado. Veja [store-state-spec](../../3-reference/frontend/store-state-spec.md).
- **Header HTTP / Interceptor Axios:** Injeção do contexto do tenant ativo em todas as chamadas API para o backend.

---

## Regras de Negócio Associadas
- [multi-tenancy-strategy](../architecture/multi-tenancy-strategy.md): Estratégia de isolamento por empresa.
