...
> **Data:** Julho 2026
> **Decisor:** Rafael
> **Relacionados:** [ADR-012: Geração Automática da Camada de API do Frontend via Orval](012-orval-contract-driven-frontend.md) · [ADR-018: Testes de Fim a Fim (E2E) com Playwright](018-playwright-e2e-testing.md) · [Smart vs Dumb Components](../concepts/smart-dumb-components.md)
...
### 2.1 Smart Components (Containers ou Orchestrators)
...
### 2.2 Dumb Components (Presenters ou Views)
...
### 2.3 Funções Utilitárias Puras (Helpers)
...
### 2.4 Resolução de Import e Mocking sob `isolate: false`
- Evita-se o uso de `vi.mock` local em arquivos de teste para hooks customizados. Centralize os mocks em `test-setup.ts` via `registerMockHook`.
...
- **Multiplicação de Arquivos:** O padrão exige a criação de mais arquivos por feature (ex: `MyPage.tsx`, `MyPageView.tsx`, `useMyPageForm.ts`), o que é mitigado pela estrutura de pastas por features bem documentada e convenções uniformes.
...
2. [ADR-018: Testes de Fim a Fim (E2E) com Playwright](018-playwright-e2e-testing.md)
3. [Smart vs Dumb Components](../concepts/smart-dumb-components.md)
