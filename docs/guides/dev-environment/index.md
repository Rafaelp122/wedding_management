...
> **Categoria:** [how-to](../../index.md) | [setup-local-environment](setup-local-environment.md) | [database-migrations](database-migrations.md)
...
graph TD
    subgraph Host ["Máquina do Desenvolvedor (Host Local)"]
        ...
        Makefile["Makefile Orquestrador<br/>(make up, dev, sync-api, test)"]
    end
...
| Serviço | URL Local | Porta Host | Comando Makefile | Descrição |
| :--- | :--- | :--- | :--- | :--- |
| **Backend & Swagger API** | [`http://localhost:8000/api/v1/docs`](http://localhost:8000/api/v1/docs) | `8000` | `make up` / `make dev` | Documentação OpenAPI interativa do Django Ninja |
| **Frontend SPA** | [`http://localhost:5173`](http://localhost:5173) | `5173` | `make frontend-dev` | Interface SPA React 19 para casais e cerimonialistas |
| **Landing Page Comercial** | [`http://localhost:4321`](http://localhost:4321) | `4321` | `make landing-dev` | Portal público estático em Astro com ilhas React |
| **Documentação Técnica** | [`http://localhost:8001`](http://localhost:8001) | `8001` | `make docs-dev` | Portal MkDocs Material com recarregamento em tempo real |
| **PostgreSQL Database** | `localhost:5432` | `5432` | `make up` | Banco de dados relacional principal |
...
2. **[Execução Segura de Migrações de Banco de Dados](database-migrations.md)**:
