# Diretrizes de Design do Sistema (Sim, Aceito!)

Este documento estabelece a especificação do sistema de design, tokens visuais, padrões de interação (UX/UI) e regras de desenvolvimento frontend adotados no **Sim, Aceito!**, servindo como guia oficial para novas implementações, refatorações e geração de interfaces por agentes.
...
- **Estado de Erro (Vermelho/Urgente)**: Aplicado quando o volume de pendências é **maior que zero** em métricas de risco imediato (ex: _Parcelas Vencidas_ ou _Tarefas Atrasadas_).
...
- **Estado de Atenção (Amarelo/Alerta)**: Aplicado a métricas que representam negociações pendentes de médio prazo (ex: _Contratos Pendentes_).
